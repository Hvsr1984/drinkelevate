...
import { playWaterDropSound } from "@/lib/sounds";
          ...
          {items.length === 0 ? (
            ...
            <>
                  ...
                  {items.map((item) => (
                    <div key={item.variantId} className="flex gap-4 p-3 rounded-lg bg-secondary/30">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="font-semibold text-primary text-sm">₹{parseFloat(item.price.amount).toFixed(0)}</p>
                      </div>
              ...
              <div className="flex-shrink-0 space-y-4 pt-4 border-t border-border">
                ...
                <Button onClick={handleCheckout} className="w-full bg-gradient-gold text-primary-foreground hover:opacity-90" size="lg" disabled={items.length === 0 || isLoading || isSyncing}>
